...
import pandas as pd
from indicators import structure, supertrend, volume_confirmation, wyckoff_heuristic, candle_bias_1h, rsi_divergence_detail
...
def score_snapshot(df12, df4, df1h, df1d, vix_bias='neutral', asset_type='crypto'):
    L=S=0.0
    structs={}
    for tf,df,w in [('12h',df12,14),('4h',df4,11),('1d',df1d,8)]:
    ...
    for df,w in [(df12,8),(df4,6)]:
        _,d=supertrend(df,10,3.0)
        if int(d.iloc[-1])==1:L+=w
        else:S+=w
    for df,maxp in [(df12,10),(df4,7)]:
        b=bos_choch(df)
    ...
    if d4['type']=='bullish':L+=6*d4['strength']
    elif d4['type']=='bearish':S+=6*d4['strength']
    if d12['type']=='bullish':L+=9*d12['strength']
    elif d12['type']=='bearish':S+=9*d12['strength']
    if d4['type']==d12['type'] and d4['type']!='none':
        if d4['type']=='bullish':L+=5
        else:S+=5
    fib=impulse_fib(df12)
    if fib.get('status')=='healthy_pullback':
        pts=14*(.75+.25*fib.get('impulse_quality',.5))
    ...
    elif wy=='markdown':S+=5
    vr=volume_confirmation(df4)
    if vr>=1.25:
        if structs['4h']=='bullish':L+=5
        elif structs['4h']=='bearish':S+=5
    ...
    conflict=min(L,S)*.42; L=max(0,L-conflict); S=max(0,S-conflict)
    ls=min(100,L/128*100); ss=min(100,S/128*100)
